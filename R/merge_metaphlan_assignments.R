...
#' Merges two or more Metaphlan-like taxonomical assignments. The input data frames are assumed to
...
#'
#' @export
merge_metaphlan_assignments <- function(assignments) {
  ...
  #
  res <- dplyr::select(assignments[[1]], LEVEL, TAXA, ROLLUP)
  ...
  if (length(assignments) > 1) {
    for (i in 2:length(assignments)) {
      res  <- base::merge.data.frame(res, dplyr::select(assignments[[i]], LEVEL, TAXA, ROLLUP),
