...
#'
#' @param assignment The DIAMOND-like assignment table.
...
#'
#' @export
plot_diamond_assignment <- function(assignment, level, plot_title, column_title, filename) {

  LEVEL <- TAXA <- COUNT <- tool <- NULL # fix the CRAN note

  # subset rows
  sub_table <- dplyr::filter(assignment, LEVEL == level)

  # subset columns
  vals <- dplyr::select(sub_table, TAXA, COUNT)

  # sort rows by the abundance
  vals <- dplyr::arrange(vals, COUNT)
  ...
  p <- ggplot2::ggplot( data = vals, ggplot2::aes(y = TAXA, x = tool, fill = COUNT) ) +
       ggplot2::theme_bw() +
