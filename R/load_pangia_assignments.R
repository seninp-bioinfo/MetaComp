...
#' Efficiently loads a PANGIA (or other EDGE-like taxonomic assignment) tables from a list
...
#'
#' @export
load_pangia_assignments <- function(filepath) {
  ...
  # read files
  #
  input_assignments_list <- list(MetaComp::load_pangia_assignment(df[1, ]$V2))
  if (dim(df)[1] > 1) {
    for (i in 2:(dim(df)[1])) {
      input_assignments_list <- c(input_assignments_list,
                                     list(MetaComp::load_pangia_assignment(df[i, ]$V2)))
    }
  }
